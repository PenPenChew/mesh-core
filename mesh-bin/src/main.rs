...
async fn main() -> anyhow::Result<()> {
    ...
    info!("Starting mesh node with ID: {}", args.node_id);
    ...
    let (_grpc_server_handle, _session_manager_handle, _delivery_queue, manager_event_tx, session_registry, topology_update_tx, mut received_topology_rx, mut session_command_rx) = if args.enable_grpc {
        ...
        let (grpc_server, incoming_message_tx) = MeshGrpcServerBuilder::new()
        ...
        info!("Starting gRPC server on {}", args.grpc_bind);
    ...
    loop {
        tokio::select! {
            ...
            SessionEvent::Connected {
                ...
                println!(
                    "[session] Connected to {} as peer_node={}",
                    peer, remote_node_id
                );
                ...
                tokio::spawn(async move {
                    ...
                    println!("[topology] Routing table updated after connection to node {}", remote_node_id);
            ...
            SessionEvent::Disconnected { remote_node_id } => {
                if let Some(node_id) = remote_node_id {
                    println!("[session] Disconnected from peer_node={}", node_id);
                    ...
                    tokio::spawn(async move {
                        ...
                        drop(db);
                        routing_table_clone.update_routes_from_topology(&computed_routes).await;
                        println!("[topology] Routing table updated after disconnection from node {}", node_id);
            ...
            SessionEvent::Pong {
                ...
                println!("[keepalive] peer_node={} rtt={:?}", remote_node_id, rtt);
            }
            SessionEvent::MessageReceived { message } => {
                ...
                if manager_event_tx.is_none() {
                    println!("[message] Note: SessionManager not running - message not routed");
                }
            }
            SessionEvent::TopologyUpdate { update } => {
                println!(
                    "[topology] Received topology update from node {} (seq: {}, {} neighbors)",
                ...
                if manager_event_tx.is_none() {
                    ...
                    tokio::spawn(async move {
                        ...
                        if db.process_topology_update(update) {
                            ...
                            routing_table_clone.update_routes_from_topology(&computed_routes).await;
                            println!("[topology] Routing table updated with new topology");
                        }
                    });
                } else {
            ...
            SessionEvent::TopologyRequest { request } => {
                println!(
                    "[topology] Received topology request from node {} (target: {:?})",
            ...
            Some(topology_update) = async {
                ...
                println!(
                    "[topology] Processing received topology update from node {} (seq: {}, {} neighbors)",
                ...
                tokio::spawn(async move {
                    ...
                    if db.process_topology_update(topology_update.clone()) {
                        ...
                        drop(db); // Release lock before async call
                        routing_table_clone.update_routes_from_topology(&computed_routes).await;
                        println!("[topology] Routing table updated with received topology from node {}", topology_update.originator_node);
