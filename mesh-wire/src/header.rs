...
pub enum FrameType {
    /// Data frame
    Data = 0x00,
    /// Mesh acknowledgment frame
    MeshAck = 0x01,
    /// Client acknowledgment frame
    ClientAck = 0x02,
    ...
    TopologyRequest = 0x0B,
    
    // ===================== Gossip & Membership (0x20..0x2F) =====================
    /// Compressed digest exchange (ids + versions) to decide what to pull
    GossipSummary = 0x20,
    /// The actual missing keys/updates requested by peer
    GossipDelta = 0x21,
    /// Ask peer to send deltas for specific topics/versions
    GossipPullReq = 0x22,
    /// Response with requested deltas (same body as GossipDelta)
    GossipPullResp = 0x23,
    /// SWIM: advertise/self-alive with incarnation
    MembershipAlive = 0x24,
    /// SWIM: mark node suspected (gossip)
    MembershipSuspect = 0x25,
    /// SWIM: confirm failure/leave
    MembershipConfirmed = 0x26,
    /// Node join request/announce (domain aware)
    MembershipJoin = 0x27,
    /// Graceful leave/retire
    MembershipLeave = 0x28,
    /// SWIM direct probe (payload can piggyback digests)
    ProbePing = 0x29,
    /// Response to PING (include measured RTT echo)
    ProbeAck = 0x2A,
    /// SWIM indirect probe (ask C to ping B)
    ProbePingReq = 0x2B,
    /// Epidemic/broadcast layer (PlumTree-style)
    BroadcastMsg = 0x2C,
    /// Backpressure/prune redundant push edges
    BroadcastPrune = 0x2D,
    /// Optional: bloom-filtered summary to cut bytes
    StateSummaryBloom = 0x2E,
    /// Negative responses / unsupported/overflow
    GossipNack = 0x2F,

    // ========================= Consensus (0x30..0x3F) ==========================
    /// Raft PreVote request
    ConsPrevoteReq = 0x30,
    /// Raft PreVote response
    ConsPrevoteResp = 0x31,
    /// Raft RequestVote request (same fields as PreVote)
    ConsVoteReq = 0x32,
    /// Raft RequestVote response
    ConsVoteResp = 0x33,
    /// Raft AppendEntries (heartbeat/data)
    ConsAppendEntries = 0x34,
    /// Response to AppendEntries
    ConsAppendResp = 0x35,
    /// InstallSnapshot chunk
    ConsInstallSnapshot = 0x36,
    /// Acknowledge snapshot chunk / flow control
    ConsSnapshotAck = 0x37,
    /// Ask follower to start election (leadership transfer)
    ConsTimeoutNow = 0x38,
    /// Linearizable read flow request
    ConsReadIndexReq = 0x39,
    /// Linearizable read flow response
    ConsReadIndexResp = 0x3A,
    /// Joint consensus/config change
    ConsConfigChange = 0x3B,
    /// Client → leader proposal envelope
    ConsClientPropose = 0x3C,
    /// Client result/redirect
    ConsClientResult = 0x3D,
    /// Term bump/role mismatch/backpressure
    ConsNack = 0x3E,
    /// Future consensus variants (EPaxos etc.)
    ConsExtension = 0x3F,
...
impl TryFrom<u8> for FrameType {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            0x01 => Ok(FrameType::MeshAck),
            0x02 => Ok(FrameType::ClientAck),
            0x03 => Ok(FrameType::Credit),
            0x04 => Ok(FrameType::Ping),
            0x05 => Ok(FrameType::Pong),
            0x06 => Ok(FrameType::Hello),
            0x07 => Ok(FrameType::Resume),
            0x08 => Ok(FrameType::Drain),
            0x09 => Ok(FrameType::Bye),
            0x0A => Ok(FrameType::TopologyUpdate),
            0x0B => Ok(FrameType::TopologyRequest),
            0x20 => Ok(FrameType::GossipSummary),
            0x21 => Ok(FrameType::GossipDelta),
            0x22 => Ok(FrameType::GossipPullReq),
            0x23 => Ok(FrameType::GossipPullResp),
            0x24 => Ok(FrameType::MembershipAlive),
            0x25 => Ok(FrameType::MembershipSuspect),
            0x26 => Ok(FrameType::MembershipConfirmed),
            0x27 => Ok(FrameType::MembershipJoin),
            0x28 => Ok(FrameType::MembershipLeave),
            0x29 => Ok(FrameType::ProbePing),
            0x2A => Ok(FrameType::ProbeAck),
            0x2B => Ok(FrameType::ProbePingReq),
            0x2C => Ok(FrameType::BroadcastMsg),
            0x2D => Ok(FrameType::BroadcastPrune),
            0x2E => Ok(FrameType::StateSummaryBloom),
            0x2F => Ok(FrameType::GossipNack),
            0x30 => Ok(FrameType::ConsPrevoteReq),
            0x31 => Ok(FrameType::ConsPrevoteResp),
            0x32 => Ok(FrameType::ConsVoteReq),
            0x33 => Ok(FrameType::ConsVoteResp),
            0x34 => Ok(FrameType::ConsAppendEntries),
            0x35 => Ok(FrameType::ConsAppendResp),
            0x36 => Ok(FrameType::ConsInstallSnapshot),
            0x37 => Ok(FrameType::ConsSnapshotAck),
            0x38 => Ok(FrameType::ConsTimeoutNow),
            0x39 => Ok(FrameType::ConsReadIndexReq),
            0x3A => Ok(FrameType::ConsReadIndexResp),
            0x3B => Ok(FrameType::ConsConfigChange),
            0x3C => Ok(FrameType::ConsClientPropose),
            0x3D => Ok(FrameType::ConsClientResult),
            0x3E => Ok(FrameType::ConsNack),
            0x3F => Ok(FrameType::ConsExtension),
